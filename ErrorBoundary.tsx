                ...
                <tr key={client.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gray-200 text-gray-900 flex items-center justify-center font-bold text-lg">
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-lg font-medium text-gray-900">هیچ مشتەرییەک نەدۆزرایەوە</p>
                      <p className="text-sm text-gray-500 mt-1">وشەیەکی تر تاقی بکەرەوە یان مشتەری نوێ زیاد بکە.</p>
                    </div>
                  </td>
                </tr>
              )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">ناوی مشتەری <span className="text-red-500">*</span></label>
                  <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-shadow" placeholder="ناوی تەواو" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">ناوی کۆمپانیا <span className="text-red-500">*</span></label>
                  <input required type="text" value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} className="w-full p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-shadow" placeholder="ناوی کۆمپانیا یان براند" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">ژمارە مۆبایل</label>
                  <input type="text" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-shadow" dir="ltr" placeholder="0770 000 0000" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">ئیمەیڵ</label>
                  <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-shadow" dir="ltr" placeholder="example@domain.com" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">تێبینی زیاتر</label>
                <textarea value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-shadow h-24 resize-none" placeholder="هەر زانیارییەکی تر..."></textarea>
              </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 text-center shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">سڕینەوەی مشتەری</h3>
            <p className="text-gray-500 mb-8">ئایا دڵنیایت لە سڕینەوەی ئەم مشتەرییە؟ ئەم کردارە ناگەڕێتەوە و هەموو داتاکانی پەیوەست بەم مشتەرییە لەدەست دەچن.</p>
